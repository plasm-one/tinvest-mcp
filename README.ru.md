...
<p align="center">
  <strong>Локальный MCP-сервер для брокерского API Т-Банк Инвестиции (T-Invest).</strong><br>
  Ресёрч, аналитика портфеля, риск-проверенные превью заявок, исполнение через гейты.<br>
...
MCP-сервер ([Model Context Protocol](https://modelcontextprotocol.io)), который
работает **на вашей машине** и даёт ИИ-ассистенту 35 инструментов для работы с
брокерским счётом T-Invest: чтение портфеля, скрининг каталога инструментов,
расчёт доходности и риска, построение плана ребалансировки и — за явными
гейтами — выставление лимитных заявок.
